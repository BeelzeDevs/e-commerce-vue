<template>
    <div class="max-h-[100dvh] flex bg-bgContent">
        <AdminSidebar />

        <div class="p-6 md:pt-28 flex flex-col items-center justify-start   bg-bgContent w-full text-white border border-x-0 border-y-adminborders overflow-y-auto overscroll-contain custom-scrollbar ">
            <form @submit.prevent="handleCrearUsuario" action="" class="bg-slate-800 p-8 rounded-2xl shadow space-y-5 text-white  border-2 border-blue-600  max-w-[450px] w-full">

                <h2 class="text-xl font-semibold text-center font-mono block ">Crear Admin</h2>
            ...
            <div class="rounded-2xl max-w-[450px] w-full flex gap-4 mt-5 flex-col items-stretch p-8 bg-slate-800 border-2 border-blue-600 shadow text-white" v-if="errorList.length > 0" >
...
import { type UsuarioReadDTO, type CreateAdminDTO, esResultError } from '@/dtos/DTOs';
...
const UsuarioACrear = ref<CreateAdminDTO>({
  nombre : "",
  email : "",
  password : ""
});
...
const CrearUsuario = async() =>{
  const resp = await fetchApi<UsuarioReadDTO>('Usuario/crear-admin',{
    method : "POST",
    body : JSON.stringify(UsuarioACrear.value)
  });
  if(esResultError(resp.results)) console.log('Error al crear usuario : ' + resp.results.errorMessage);
  else{
